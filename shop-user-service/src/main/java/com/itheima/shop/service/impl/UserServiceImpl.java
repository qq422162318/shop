...
public class UserServiceImpl implements IUserService{
    ...
    @Override
    public Result reduceMoneyPaid(TradeUserMoneyLog userMoneyLog) {
        //1.校验参数是否合法
        ...
        int r=userMoneyLogMapper.countByExample(userMoneyLogExample);
        TradeUser tradeUser=userMapper.selectByPrimaryKey(userMoneyLog.getUserId());
        if (userMoneyLog.getUseMoney().intValue()==ShopCode.SHOP_USER_MONEY_PAID.getCode().intValue()){
          if (r>0){
              //已经付款
              CastException.cast(ShopCode.SHOP_ORDER_PAY_STATUS_IS_PAY);
          }
        ...
        if (userMoneyLog.getUseMoney().intValue()==ShopCode.SHOP_USER_MONEY_REFUND.getCode().intValue()) {
            if (r < 0) {
                CastException.cast(ShopCode.SHOP_ORDER_PAY_STATUS_NO_PAY);
            }
            //防止多次退款
            TradeUserMoneyLogExample userMoneyLogexample=new TradeUserMoneyLogExample();
            TradeUserMoneyLogExample.Criteria criteriaexample=userMoneyLogexample.createCriteria();
            criteriaexample.andOrderIdEqualTo(userMoneyLog.getUserId());
            criteriaexample.andUserIdEqualTo(userMoneyLog.getUserId());
            criteriaexample.andMoneyLogTypeEqualTo(ShopCode.SHOP_USER_MONEY_REFUND.getCode());
            int rthread=userMoneyLogMapper.countByExample(userMoneyLogexample);
            if (rthread>0){
                CastException.cast(ShopCode.SHOP_USER_MONEY_REFUND_ALREADY);
            }
            tradeUser.setUserMoney(new BigDecimal(tradeUser.getUserMoney()).add(userMoneyLog.getUseMoney()).longValue());
